...
export function PortfolioShowcase() {
  ...
  const projects = [
    ...
    {
      title: "Full Stack CRM Business Platform",
      category: "Web Development",
      description:
        "Comprehensive enterprise CRM solution featuring customer relationship management, sales pipeline tracking, advanced admin dashboard, real-time analytics, automated workflows, and payment processing. Built with modern full-stack technologies for maximum scalability and performance.",
    ...
    },
    {
      ...
      category: "AI/ML",
      description:
        "Enterprise-grade AI chatbot solutions with conversational automation, full-system integrations, and multi-channel deployment. Transform customer communication and automate operations with intelligent digital employees.",
      image: "/placeholder.svg?height=300&width=600",
      technologies: [
        "Python",
        "OpenAI GPT",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {filteredProjects.map((project, index) => {
            const isCRM = project.title === "Full Stack CRM Business Platform"
            const isChatbot = project.title === "AI Chatbots & Intelligent Automation"
            
            const CardWrapper = isCRM
              ? ({ children }: { children: ReactNode }) => (
                  <Link href="/portfolio/crm-business-platform" className="block h-full">
                    {children}
                  </Link>
                )
              : isChatbot
              ? ({ children }: { children: ReactNode }) => (
                  <Link href="/portfolio/ai-chatbots" className="block h-full">
                    {children}
                  </Link>
                )
            ...
            return (
              <CardWrapper key={project.title}>
                <Card
                  className={`group hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border-2 hover:border-primary/20 bg-card/50 backdrop-blur-sm overflow-hidden h-full ${
                    isCRM || isChatbot ? "cursor-pointer" : ""
                  }`}
